...
au WinEnter Buffer\ List cal<SID>ActivateBufPane()

nn <silent> gl :cal<SID>ActivateBufPane()<cr>

fun s:ActivateBufPane()
...
fun s:CreateBufPane()
	...
	syn match bufpaneNum '^\d\+:' contained
	syn match bufpaneString '^\d\+:\zs[^\.]*' contained
	syn match bufpaneExt '\(\.\| (\).*' contained
	syn match bufpaneEntry '^\d\+:.*' contains=bufpaneNum,bufpaneString,bufpaneExt
	syn match bufpaneComment '^\D.*$'
